  ...
  _responseHandler: function(response) {
    ...
    if (this._isRedirect(response) && this.options.followRedirects == true) {
      ...
      response.addListener('end', function() {
        ...
        if (self.options.parser && statusCode <= 400) body = self.options.parser.call(response, body);

        sys.debug(parseInt(response.statusCode));

        if (statusCode >= 400) self._respond('error', body, response);
